...
pte_t *pt_addr;		// Physical address in PT
	...
	for (i = 0; i < 1024; i++) {
		...
		if ((((ptr->mm->pgd) + i)->pgd) != 0x0) {
			...
			if (i < 768) {
				...
				pgd_phaddr = (pgd_t *)(((ptr->mm->pgd) + i)->pgd);
				pt_addr = (pte_t *) kmap (&mem_map[pgd_phaddr >> 12]);

				for (j = 0; j < 1024; j++) {

					if ((pt_addr + j)->pte != 0x0){

					pt_phaddr = (pt_addr + j)->pte;
					...
					printk (KERN_CONT "%3p --> %8x --> %8p --> :: %8p\n", j,
					(pt_addr + j)->pte, (unsigned long) (kmap (&mem_map[pt_phaddr >> 12])),
					*((unsigned int *)((unsigned int)page_phaddr | 0x0000003c)));
					}
				}
			} /*else if (i >= 768) {
