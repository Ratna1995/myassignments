...
{
	struct task_struct *pid1 = find_task_by_vpid(pid);
	struct vm_area_struct *ptr = NULL;
	//struct fdtable __rcu *fd1 = NULL;
	//struct file __rcu **fdd = NULL;
...
//	pid1 = find_task_by_vpid(pid);	
	...
	while(ptr != NULL) {
		...
		ptr->vm_flags & 4 ? printk(KERN_CONT"x") : printk(KERN_CONT"_");
		ptr->vm_flags & 8 ? printk(KERN_CONT"s  ") : printk(KERN_CONT"p  ");
...
//			if(ptr->vm_file->f_inode->i_sb != NULL)
//			printk(KERN_CONT"%x ",ptr->vm_file->f_inode->i_sb->s_bdi->min_ratio);
		//	fd1 = pid1->files->fdt;
		//	fdd = fd1->fd;
//			printk(KERN_CONT"%0.7lu ",pid1->files->fdt->(*fd + 3)->f_mapping->host->i_rdev);
//			printk(KERN_CONT" %lu:%lu",MAJOR(ptr->vm_file->f_inode->i_rdev),MINOR(ptr->vm_file->f_inode->i_rdev));
			fun(name);
